...
top7_postulantes = [c for c in complete if str(c.get('uni_prioritaria', '')).strip().lower() in ['si', 'sí', 'true'] or is_uni_prioritaria(c)]
top7_elegibles = [c for c in top7_postulantes if is_candidate_eligible(c)]
...
print(f"--- Top 7 ---")
print(f"Postulantes Top 7 (Prioritarias): {len(top7_postulantes)}")
print(f"Elegibles Top 7: {len(top7_elegibles)}")
concentracion = (len(top7_elegibles) / len(eligible) * 100) if eligible else 0
print(f"Concentracion: {concentracion:.1f}%")
avance_meta = (len(top7_elegibles) / 300 * 100)
print(f"Avance Meta: {avance_meta:.1f}%")
