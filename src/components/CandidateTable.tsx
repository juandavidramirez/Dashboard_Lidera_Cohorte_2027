...
export const CandidateTable: React.FC<Props> = ({
  candidates,
  title = "Registros del Formulario de Convocatoria LIDERA (Cohorte 2027)",
  subtitle,
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden mb-8">
      ...
      <div className="p-4 bg-slate-50 border-b border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <div className="mt-2 flex items-center gap-2 flex-wrap">
            ...
            {isIncompleteMode && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-bold bg-emerald-50 text-emerald-900 border border-emerald-300 shadow-2xs">
                <Sparkles className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                Potenciales Elegibles: <strong className="font-black text-emerald-950">{filteredCandidates.filter((c) => isCandidateEligible(c)).length}</strong> de {filteredCandidates.length}
              </span>
            )}
      ...
      {isIncompleteMode && (
        <div className="mx-4 mt-4 p-3.5 bg-amber-50 border border-amber-300/80 rounded-lg text-xs text-amber-950 flex items-start gap-2.5 shadow-2xs">
          <Info className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
          <div>
            <p className="font-extrabold text-amber-900">
              Registros con Formulario Incompleto
            </p>
            <p className="text-[11px] text-amber-800 mt-0.5 leading-relaxed">
              Estos postulantes aún no han completado todas las secciones del formulario de postulación en la plataforma.
              <strong className="ml-1 text-amber-950 font-black">
